 </div>
        <!-- /#page-wrapper -->
    </div>
    <!-- /#wrapper -
<!-- Core Scripts - Include with every page -->
    <script src="<?php echo base_url()?>/application/assets/js/jquery-1.10.2.js"></script>
      <script src="<?php echo base_url()?>/application/assets/js/wow.min.js"></script>
      
              <script>
              new WOW().init();
              </script>
        <script src="<?php echo base_url()?>application/assets/js/parsley.js" type="text/javascript"></script>      
    <script src="<?php echo base_url()?>/application/assets/js/bootstrap.min.js"></script>
    <script src="<?php echo base_url()?>/application/assets/js/plugins/metisMenu/jquery.metisMenu.js"></script>
    <script src="<?php echo base_url()?>/application/assets/ckeditor/ckeditor.js"></script>

    <!-- Page-Level Plugin Scripts - Dashboard -->
    <script src="<?php echo base_url()?>/application/assets/js/plugins/morris/raphael-2.1.0.min.js"></script>
    <script src="<?php echo base_url()?>/application/assets/js/plugins/morris/morris.js"></script>

    <!-- SB Admin Scripts - Include with every page -->
    <script src="<?php echo base_url()?>/application/assets/js/sb-admin.js"></script>

    <!-- Page-Level Demo Scripts - Dashboard - Use for reference -->
    <script src="<?php echo base_url()?>/application/assets/js/demo/dashboard-demo.js"></script>
  </body>
  </html>